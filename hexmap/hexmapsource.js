...
* @param {olx.source.HexMap=} options
* @todo 
*/
ol.source.HexMap = function(options)
{	var options = options || {};
...
*/
ol.source.HexMap.prototype.drawHexagon = function (ctx, p, size, fill, min, max, dl)
{	var p0;
	ctx.beginPath();
	min = min || 0;
	max = max || 6;
...
*/
ol.source.HexMap.prototype.showCoordiantes = function (what)
...
* @API stable
*/
ol.source.HexMap.prototype.drawHex = function (extent, res, ratio, size, proj)
{	var w = this.canvas.width = size[0];
	...
	{	var pattern = document.createElement('canvas');
		...
		for (var x=-psize; x<2*psize+2; x++)
			for (var y=-3; y<2*psize+2; y++)
			{	var p = this.grid.hex2coord([h0[0]+x,h0[1]+y]);
	...
	{	var ctx = this.context;
		ctx.save();
			...
			// draw
			for (var x=o0[0]-1; x<=o1[0]+1; x++)
			{	for (var y=o0[1]-1; y<=o1[1]+1; y++)
				{	var hex = this.grid.offset2hex([x,y], this.layout_);
					var p = this.grid.hex2coord(hex);
					// Coord to pixel
					p[0] = (p[0] - extent[0])/res;
					...
					this.drawHexagon(ctx, p, pxSize, false, 5, 8, 2 );
					// Show coords
					if (pxSize > 20) switch (this.coordinates_)
					{	case 'axial': 
							ctx.fillText(hex[0]+","+hex[1], p[0], p[1]); 
						break;
						case 'cube': 
							var c = this.grid.hex2cube(hex);
							//c=["x","y","z"]
							for (var a=0; a<3; a++)
						...
						break;
						case 'offset': 
							var c = this.grid.hex2offset(hex);
